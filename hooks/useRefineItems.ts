  ...
  const refinedItems = useMemo(() => {
    ...
    if (search || filterByType) {
      refinableItems = itemsCopy.filter((item) => {
        if (search && filterByType && item.name.includes(search) && item.types.includes(filterByType)) {
          return item;
        }
